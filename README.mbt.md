...
闭曲面满足欧拉示性数 V − E + F = 2 − 2g。三个样例**分毫不差**。这同时验证了
顶点表下标没读错、容差焊接没焊错、水密性判断是对的——下标只要偏一位，
这几个数立刻就崩。

最后一行还多验了一层：那个文件里只有**一个**对象，但 build 里放了 **6 个
item**，各带一个不同的平移变换，所以最终是 6 个互不相连的闭合体。**同一份
网格被引用多次、每次套不同变换**这条路径也一并验过了。

### 官方一致性测试：13 个全过

3MF Consortium 除了示例模型，还出了一套一致性测试（`3mf-Verify`）。
其中 `MUSTPASS` 目录的意思是：**凡是符合规范的读取器，必须能正确处理这些文件**。
正好拿来当外部标尺——不是我造几个测试夸自己，是官方出的题。

13 个文件全部通过，都收在 `examples/conformance/` 下，可以自己跑。几个特别值得看：

- **只输出被引用的对象**（`Chapter3.4.3a`）：文件里有 2 个对象，但 `<build>` 只引用
  了 1 个。工具输出 12 个三角形而不是 24 个——没偷懒把 resources 全倒出来
- **两条不同路径给出同一个结果**：`Chapter3.2c`（1 个对象 + 2 个 item）和
  `Chapter4.2`（2 个对象 + 2 个 component）用 3MF 里两种完全不同的机制描述同一几何，
  结果**逐位相同**（60 个三角形、14000.003 mm³）。这两条路径在实现里是分开的
  （一个是 build 层的重复引用，一个是对象层的嵌套引用，后者还要做矩阵合成），
  能对上说明两边都对
- **无关内容不干扰解析**：`Chapter2.3a`（可忽略标记）和 `Chapter3.4.1c`
  （未定义的 metadata 名）往文件里塞了一堆和几何无关的东西，解析器不能被带偏
